...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  int _getTodayCompletedCount(HabitProvider provider) {
    final now = DateTime.now();
    if (provider.habitState == null) return 0;
    
    int count = 0;
    for (final habit in provider.activeHabits) {
      if (habit.dailyLogs.isNotEmpty && 
          now.day <= habit.dailyLogs.length && 
          habit.dailyLogs[now.day - 1]) {
        count++;
      }
    }
    return count;
            ...
            children: [
              ...
              Expanded(
                child: StatsCard(
                  ...
                  value: _getTodayCompletedCount(provider).toString(),
    ...
    return ChangeNotifierProvider<HabitProvider>(
      ...
      child: Consumer<HabitProvider>(
        builder: (context, provider, child) {
          return Scaffold(
            appBar: AppBar(
              title: Text('Welcome, ${Provider.of<AuthProvider>(context).user?.email ?? "User"}'),
